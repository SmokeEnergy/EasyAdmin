...
AddEventHandler('EasyAdmin:requestCachedPlayers', function(playerId)
	...
	if DoesPlayerHavePermission(source,"easyadmin.ban2") then
		TriggerClientEvent("EasyAdmin:fillCachedPlayers", src, CachedPlayers)
		PrintDebugMessage("Cached Players requested by "..getName(src,true))
	end		
end)
...
Citizen.CreateThread(function()
	...
	AddEventHandler('EasyAdmin:banPlayer', function(playerId,reason,expires,username)
		if playerId ~= nil then
			...
			if DoesPlayerHavePermission(source,"easyadmin.ban2") and CachedPlayers[playerId] and not DoesPlayerHavePermission(playerId,"easyadmin.immune") then
				local bannedIdentifiers = CachedPlayers[playerId].identifiers or GetPlayerIdentifiers(playerId)
				if expires and expires < os.time() then
					expires = os.time()+expires 
				elseif not expires then 
	...
	AddEventHandler('EasyAdmin:TakeScreenshot', function(playerId)
		...
		if DoesPlayerHavePermission(source,"easyadmin.screenshot") then
			thistemporaryevent = AddEventHandler("EasyAdmin:TookScreenshot", function(result)
				...
				print('res = ' ..resp.files[1].url.. '')
				if (moderationNotification == GetConvar("ea_screenshoturl", 'http://screen.seven-life-rp.ovh:3555/upload')) then
					res = ""
				end
				SendWebhookMessage(moderationNotification, string.format(GetLocalisedText("admintookscreenshot"), getName(src), getName(playerId), resp.files[1].url), "screenshot")
				TriggerClientEvent('chat:addMessage', src, { template = '<img src="{0}" style="max-width: 400px;" />', args = { resp.files[1].url } })
				TriggerClientEvent("chat:addMessage", src, { args = { "EasyAdmin", string.format(GetLocalisedText("screenshotlink"), resp.files[1].url) } })
				PrintDebugMessage("Screenshot for Player "..getName(playerId,true).." done, "..resp.files[1].url.." requsted by"..getName(src,true))
				scrinprogress = false
